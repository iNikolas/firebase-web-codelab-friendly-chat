...
function isUserSignedIn() {
    // TODO 6d: Return true if a user is signe-in.
...
function loadMessages() {
    ...
    onSnapshot(recentMessagesQuery, (snapshot) => {
        snapshot.docChanges().forEach(change => {
            if (change.type === 'removed') return deleteMessage(change.doc.id)
...
async function saveMessagingDeviceToken() {
    ...
    try {
        ...
        if (currentToken) {
            ...
            return onMessage(getMessaging(), message => console.log('New foreground notification from Firebase Messaging!', message.notification))
        }
...
async function requestNotificationsPermissions() {
    ...
    if (permission === 'granted') {
        console.log('Notification permoission granted.')

        return await saveMessagingDeviceToken()
    }

    console.log('Únable to get permission to notify')
}
